...
def test_mvn_deploy_workflow(
    ...
    # Assert that the latest version is 9 (6 artifacts + 3 version-level SNAPSHOT metadata
    # files, all including checksums). Repo-level maven-metadata.xml uploads are acknowledged
    # with a 201 but generated by Pulp itself, so they do not create repository versions.
    repo = maven_repo_api_client.read(repo.pulp_href)
    assert repo.latest_version_href.endswith("/versions/9/")
